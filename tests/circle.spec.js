...
*/
const anyfunction = (object) => {
  let area = object.area.toPrecision(4);
  console.log(area);
  object['area'] = parseFloat(area); 
  return object;
} 

anyfunction(circle(3));
...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados', () => {
    ...
    expect(anyfunction(circle(3))).toMatchObject({radius: 3, area: 28.26, circumference: 18.84});
  });
});
